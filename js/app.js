...
async function switchView(name) {
  ...
  document.getElementById('page-title').textContent = VIEWS[name]?.title || name;
  // Referme la sidebar sur mobile après sélection.
  document.getElementById('sidebar')?.classList.remove('open');
  if (VIEWS[name]) await VIEWS[name].load();
}
...
function renderOverview(health) {
  ...
  const stats = [
    { label: 'Nœuds', value: state.nodes.length, icon: 'server', color: 'indigo' },
    { label: 'En ligne', value: online, icon: 'wifi', color: 'emerald' },
    { label: 'Utilisateurs', value: state.users.length, icon: 'users', color: 'violet' },
    { label: 'Clés Pre-Auth', value: state.preauthkeys.length, icon: 'key-round', color: 'cyan' },
  ];
  const colorMap = {
    indigo: 'from-indigo-500/20 to-transparent', emerald: 'from-emerald-500/20 to-transparent',
    violet: 'from-violet-500/20 to-transparent', cyan: 'from-cyan-500/20 to-transparent',
  };
  const statsHtml = stats.map((s) => `
    <div class="card card-pad stat-card">
      <div class="flex items-center justify-between">
        <span class="text-zinc-400 text-[13px]">${s.label}</span>
        <i data-lucide="${s.icon}" class="w-5 h-5 text-zinc-500"></i>
      </div>
  ...
  const healthHtml = health
    ? `<div class="flex items-center gap-3">
        <span class="dot ${health.databaseConnectivity ? 'dot-success' : 'dot-danger'}"></span>
        <div>
          <div class="font-medium">Base de données : ${health.databaseConnectivity ? 'Connectée' : 'Injoignable'}</div>
          <div class="text-zinc-500 text-[12px]">Vérifiée à l'instant via GET /api/v1/health</div>
        </div>
      </div>`
    : '<div class="text-zinc-500">Santé non disponible (vérifiez la connexion).</div>';
  ...
  const userHtml = entries.length
    ? entries.map(([name, c]) => `
        <div class="flex items-center gap-3 py-2 border-b border-edge last:border-0">
          <span class="flex-1 text-zinc-300">${esc(name)}</span>
          <span class="badge badge-muted">${c} nœud${c > 1 ? 's' : ''}</span>
        </div>`).join('')
    : '<div class="text-zinc-500">Aucun nœud.</div>';
...
async function init() {
  ...
  if (cfg.baseUrl) {
    ...
    switchView('overview'); // rend la vue vide sans appel réseau
    openModal('modal-settings');
  }
}
